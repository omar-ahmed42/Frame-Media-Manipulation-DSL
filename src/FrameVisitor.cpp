...
#pragma warning(disable: 4996)
 ...
 antlrcpp::Any FrameVisitor:: visitResolution(FrameParser::ResolutionContext* ctx) {
     ...
     if (exists)
     {
         ...
         width.erase(std::remove(width.begin(), width.end(), ' '), width.end());
         height.erase(std::remove(height.begin(), height.end(), ' '), height.end());
         if (media.propertyApplied)
         {
             std::string TempFilePath = directory + constructName + "Temp." + media.format;
             std::string cmd = "ffmpeg" + (frm::options.overwrite ? std::string(" -y") : std::string(" -n"))
 ...
 antlrcpp::Any FrameVisitor:: visitFramerate(FrameParser::FramerateContext* ctx)  {
     ...
     if (exists)
     {
         ...
         framerate.erase(std::remove(framerate.begin(), framerate.end(), ' '), framerate.end());
         if (media.propertyApplied)
         {
             std::string TempFilePath = directory + constructName + "Temp." + media.format;
             std::string cmd = "ffmpeg" + (frm::options.overwrite ? std::string(" -y") : std::string(" -n"))
 ...
 antlrcpp::Any FrameVisitor::visitOverlaying(FrameParser::OverlayingContext* ctx) {
     ...
     if (media.type == FRAME)
     {
         std::cout << "Action at line " << ctx->start->getLine() << ": Can't  a  type";
         exit(0);
     }
...
//contrast_action: SET CONTRAST FOR NAME MINUS? contrastValue=(FLOAT | INT) (AS NAME)? (SAVE_TO STRING)? #settingContrast;
 antlrcpp::Any FrameVisitor:: visitSettingContrast(FrameParser::SettingContrastContext* ctx)  {
     std::string constructName = ctx->NAME()[0]->getText();
     //Enter action name     
     std::cout << "Changing contrast for " << constructName + "...\n";
     bool exists = false;
     int idx;
     Media media = frm::findByName(constructName, exists, idx);
     //Incompatible types     
     ...
     {
         //Error message
         std::cout << "Action at line " << ctx->start->getLine() << ": Can't change contrast for audio type";
         exit(0);
     }

     std::string directory;

     std::string contrastValue = ctx->contrastValue->getText();
     eraseSpace(contrastValue);
     
     if (std::stod(contrastValue) > 1000.0) {
         std::cout << "Action at line " << ctx->start->getLine() << ": contrast value has to be between -1000.0 and 1000.0";
         exit(0);
     }
     ...
     if (exists)
     {
         if (ctx->AS() != NULL)
         {
             ...
             //FFMPEG command
             std::string cmd = "ffmpeg" + (frm::options.overwrite ? std::string(" -y") : std::string(" -n"))
                 + " -i \"" + media.path + "\"" + " -vf \"eq=contrast=" + (ctx->MINUS() != NULL? std::string("-") : std::string("")) + contrastValue + "\"" + (frm::options.reencode ? std::string("") : std::string(" -c:a copy "))
                 + " \"" + newFilePath + "\"";
             if (frm::options.debug) std::cout << "\n\n\n" << cmd << "\n";
             else
             {
